...
csv_fields = [
    ...
    'avg_just_cosine', 'avg_no_just_cosine',
    'avg_just_euclidean', 'avg_no_just_euclidean'
]
...
for pair_id, pair in tqdm(data.items()):
    ...
    # Prepare CSV row (column order changed)
    row = [
        pair_id,
        excluded_question,
        pair['just_sentence'],
        pair['no_just_sentence'],
        json.dumps(just_questions),
