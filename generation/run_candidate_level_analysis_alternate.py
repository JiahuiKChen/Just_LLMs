...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--candidates_path",
        default=str(DEFAULT_RESULTS_DIR / "all_models_all_particles_generation_candidates.tsv"),
        help="Candidate TSV to analyze.",
    )
    parser.add_argument(
        "--output_dir",
        default=str(DEFAULT_RESULTS_DIR / "figures"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Significant figures to use when formatting CI bounds in the LaTeX table.",
    )
...
def format_ci_cell(low: float, high: float, sig_figs: int) -> str:
    if not (np.isfinite(low) and np.isfinite(high)):
        return "--"
    return f"{format_sig_figs(low, sig_figs)}--{format_sig_figs(high, sig_figs)}"
...
INSTRUCTION_LATEX_LABELS = {
    False: "Base",
    True: "Instruct",
}


def build_ci_latex_table(agg: pd.DataFrame, sig_figs: int) -> str:
    """Build a LaTeX table of 95% CIs: rows=models (grouped by family, with a
    Base/Instruct column), columns=(word, source).

    Requires the `multirow` LaTeX package (for \\multirow used on the family
    column).
    """
    table = agg.copy()
    table["ci_low"] = table["advantage"] - table["cb"]
    table["ci_high"] = table["advantage"] + table["cb"]

    words = [w for w in WORD_ORDER if w in table["word"].unique().tolist()]
    sources = ["P", "P_prime"]

    model_rows = (
        table[["short", "family", "instruction", "model_order"]]
        .drop_duplicates()
        .sort_values("model_order")
    )

    cell_lookup = {
        (row["short"], str(row["word"]), row["generated_from"]): format_ci_cell(
            row["ci_low"], row["ci_high"], sig_figs
        )
        for _, row in table.iterrows()
    }

    # One vertical rule between the family/type columns and the data, and
    # one between each word group.
    col_spec = "cl|" + "|".join("c" * len(sources) for _ in words)
    ...
    word_header_cells = []
    for word in words:
        word_header_cells.append(
            r"\multicolumn{" + str(len(sources)) + "}{c}{" + word + "}"
        )
    lines.append(" & & " + " & ".join(word_header_cells) + r" \\")

    cmidrules = []
    start_col = 3
    for _ in words:
        end_col = start_col + len(sources) - 1
    ...
    for _ in words:
        for source in sources:
            source_header_cells.append(SOURCE_LATEX_LABELS[source])
    lines.append(" & & " + " & ".join(source_header_cells) + r" \\")
    lines.append(r"\midrule")

    family_row_counts = model_rows.groupby("family", sort=False).size().to_dict()
    current_family = None
    for row in model_rows.itertuples(index=False):
        ...
        if row.family != current_family:
            ...
            n_rows = family_row_counts[row.family]
            family_cell = r"\multirow{" + str(n_rows) + r"}{*}{" + row.family + "}"

        cells = [family_cell, INSTRUCTION_LATEX_LABELS[bool(row.instruction)]]
        for word in words:
            for source in sources:
                cells.append(cell_lookup.get((row.short, word, source), "--"))
        lines.append(" & ".join(cells) + r" \\")

    lines.append(r"\bottomrule")
    lines.append(r"\end{tabular}")
    lines.append(
        r"\caption{95\% confidence intervals for the mean candidate-level own-context "
        r"advantage per token, by model, particle, and response source ($S$: with word, "
        r"$S'$: word removed.)}"
    )
    lines.append(r"\label{tab:candidate_ci}")
    lines.append(r"\end{table}")
    return "\n".join(lines) + "\n"


def build_ci_latex_table_transposed(agg: pd.DataFrame, sig_figs: int) -> str:
    """Build a transposed LaTeX table of 95% CIs: columns=models, rows=particle
    with S/S' as sub-rows underneath each particle.

    Requires the `multirow` LaTeX package (for \\multirow used on the particle
    column).
    """
    table = agg.copy()
    table["ci_low"] = table["advantage"] - table["cb"]
    ...
    model_rows = (
        table[["short", "family", "model_order"]]
        .drop_duplicates()
        .sort_values("model_order")
    )
    model_order = model_rows["short"].tolist()
    ...
    for word in words:
        for i, source in enumerate(sources):
            word_cell = (
                r"\multirow{" + str(len(sources)) + r"}{*}{" + word + "}" if i == 0 else ""
            )
            cells = [word_cell, SOURCE_LATEX_LABELS[source]]
            for model in model_order:
                cells.append(cell_lookup.get((model, word, source), "--"))
            lines.append(" & ".join(cells) + r" \\")
        lines.append(r"\midrule")
    ...
    lines.append(r"\label{tab:candidate_ci_transposed}")
    lines.append(r"\end{table}")
    return "\n".join(lines) + "\n"
...
def plot_advantage_by_model(
    ...
    source_handles = [
        ...
        for source, label in [("P", r"with word ($S$) samples"), ("P_prime", r"word removed ($S'$) samples")]
    ]
...
def main() -> None:
    ...
    ci_table_path = prefix.with_name(f"{prefix.name}_ci_table.txt")
    ci_table_path.write_text(build_ci_latex_table(agg, args.sig_figs))
    print(f"Wrote candidate-level CI LaTeX table to {ci_table_path}")

    ci_table_transposed_path = prefix.with_name(f"{prefix.name}_ci_table_transposed.txt")
    ci_table_transposed_path.write_text(build_ci_latex_table_transposed(agg, args.sig_figs))
    print(f"Wrote transposed candidate-level CI LaTeX table to {ci_table_transposed_path}")
